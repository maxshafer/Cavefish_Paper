...
data2$stratum <- factor(data2$stratum, levels = c("Shared", "All Caves", "Pachon + Tinaja", "Pachon + Molino", "Tinaja + Molino", "Pachon", "Tinaja", "Molino", "Choy-surface-morph", "Cave-morph-specific", "Surface-morph-specific", "Shared2"))
...
ggplot(data2, aes(x = x, stratum = stratum, alluvium = alluvium, y = freq, label = stratum)) + stat_flow(aes(fill = fill)) + geom_stratum() + geom_text(stat = "stratum") + scale_fill_brewer(type = "qual", palette = "Paired")
...
data4 <- data[1:7,2:4]
data5 <- data[8:15,c(3,1,4)]
colnames(data4) <- c("Var.1", "Var.2", "value")
colnames(data5) <- c("Var.1", "Var.2", "value")
data4 <- rbind(data4, data5)

nodes <- c(as.character(unique(data4$Var.2)), as.character(unique(data4$Var.1)), as.character(unique(data4$Var.2)))
nodes <- as.data.frame(nodes)

sankeyNetwork(Links = data4, Nodes = nodes, Source = "Var1", Target = "Var2", Value = "value", NodeID = "nodes2", units = "cells", fontSize = 8, nodeWidth = 40, nodePadding = 10, height = 200, width = 300, iterations = 10000)
