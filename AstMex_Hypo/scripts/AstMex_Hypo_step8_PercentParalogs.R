...
gene.lists <- readRDS("marker_gene_lists_pos.rds")
...
subtypes$Subtypes <- factor(subtypes$Subtypes, levels = levels(hypo@meta.data$Subtype))

subtypes.2 <- reshape2::melt(subtypes[,13:18])
...
paralog.numbers.sub <- lapply(seq_along(gene.lists.pos[[4]][index]), function(x) calcParalog(conserved = gene.lists.pos[[4]][index], species.1 = gene.lists.pos[[5]][index], species.2 = gene.lists.pos[[6]][index], mart.1 = mart, mart.2 = mart, i = x))

fisher.results.1 <- lapply(paralog.numbers.sub, function(x) fisher.test(matrix(data = x[1:4], ncol = 2, nrow = 2)))
fisher.results.1 <- do.call(rbind, fisher.results.1)
row.names(fisher.results.1) <- names(gene.lists.pos[[4]][index])
fisher.results.2 <- lapply(paralog.numbers.sub, function(x) fisher.test(matrix(data = x[5:8], ncol = 2, nrow = 2)))
fisher.results.2 <- do.call(rbind, fisher.results.2)
row.names(fisher.results.2) <- names(gene.lists.pos[[4]][index])

subclusters <- data.frame(do.call(rbind, paralog.numbers.sub))
row.names(subclusters) <- names(gene.lists.pos[[4]][index])
...
subclusters$Subtype <- hypo@meta.data$Subtype[match(subclusters$Subclusters, hypo@meta.data$SubclusterType)]

subclusters$Subtype <- factor(subclusters$Subtype, levels = levels(hypo@meta.data$Subtype))

subclusters.2 <- reshape2::melt(subclusters[,13:19])
