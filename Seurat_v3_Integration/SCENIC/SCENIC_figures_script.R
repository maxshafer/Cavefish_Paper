...
GO_lists <- sapply(gene.lists, function(x) union(unique(zeb.lists[[x]]$Target), unique(ast.lists[[x]]$Target)), USE.NAMES = T)
...
for (i in 1:length(ast.grns)) {
	...
	ast.grns <- ast.grns[intersect(names(zeb.grns), names(ast.grns))]
}
...
DI <- lapply(seq_along(conserved.grns), function(x) calcDriftIndex(conserved = conserved.grns[[x]], species.1 = zeb.grns[[x]], species.2 = ast.grns[[x]]))

names(DI) <- names(ast.grns)
for (i in 1:length(DI)) {
	names(DI[[i]]) <- names(ast.grns[[i]])
}
...
df <- reshape2::melt(do.call(rbind, lapply(seq_along(DI), function(x) data.frame(gene = names(DI[[x]]), GO = names(DI)[x], DI = DI[[x]], corr = correlations[[x]], percent.para = grn.paralog[[x]]$percent.para[1:length(DI[[x]])]))))

df2 <- do.call(rbind, lapply(seq_along(DI), function(x) data.frame(gene = names(DI[[x]]), GO = names(DI)[x], fisher.odds = grn.paralog[[x]]$fisher.odds.1[1:length(DI[[x]])], fisher.odds.pval = grn.paralog[[x]]$fisher.odds.pval.1[1:length(DI[[x]])])))
...
scatter2 <- ggplot(df[df$variable == "DI",], aes(x = GO, y = value, color = variable)) + geom_point(position = position_jitterdodge(), size = .5) + geom_boxplot(fill = NA, size = .5, outlier.shape = NA, color = "black") + scale_color_manual(values = c("red")) + theme_classic() + theme(axis.title.x = element_blank(), legend.title = element_blank(), axis.text.x = element_text(angle = 45, hjust = 1), axis.text = element_text(size = 8)) + scale_x_discrete(labels = c("Neuropeptides", "Neurotransmitters", "Synaptic Genes", "Ion Channel")) + guides(color = F) + geom_signif(data = anno_df[anno_df$variable == "DI",], aes(xmin = group1, xmax = group2, annotations = p.signif, y_position = y_pos), manual = T, textsize = 3) + ylab("Divergence Index (DI)")
...
## Make DI plot for TFs vs NPs/NTS

DI.list <- readRDS("../DI_results.rds")
DI.plot <- ggplot(DI.list[["DI.sub.GO"]][DI.list[["DI.sub.GO"]]$variable == c("NP_NTS", "TFs"),], aes(x = variable, y = value, group = variable, color = variable)) + geom_jitter(size = 1) + geom_boxplot(outlier.color = NA, fill = "transparent", color = "black") + scale_color_viridis_d() + xlab(element_blank()) + ylab("Cellular Drift Index") + theme_classic() + theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5, size = 8), axis.text.y = element_text(size = 8), axis.title = element_text(size = 10), legend.position = "none") + stat_compare_means()
DI.sub.GO.GABA_1 <- DI.list[[5]][grep("GABA_1", DI.list[[5]]$cell_type),]
DI.sub.GO.GABA_1$cell_type <- factor(DI.sub.GO.GABA_1$cell_type, levels = paste("GABA_1", c(0:11), sep = "_"))
DI.plot.GABA_1 <- ggplot(DI.sub.GO.GABA_1[13:36,], aes(x = cell_type, y = value, group = variable, color = variable)) + geom_point() + geom_line() + scale_color_viridis_d() + ylim(c(0,0.85)) + theme_classic() + theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5, size = 8), legend.position = "none")

## Gather plots

# Make individual dot plots per gene

dot.plots <- GO_plots[["nps"]][["oxt"]] + GO_plots[["nts"]][["syt1a"]] + GO_plots[["synaptic"]][["slc32a1"]] + GO_plots[["ion"]][["kcnc1a"]] + GO_plots[["nps"]][["nmba"]] + GO_plots[["nts"]][["gna12a"]] + GO_plots[["synaptic"]][["syt1b"]] + GO_plots[["ion"]][["kcnh4a"]] + plot_layout(nrow = 2, ncol = 4)

dot.plots <- dot.plots & theme(legend.position = "bottom")
dot.plots <- dot.plots + plot_layout(guides = "collect", nrow = 2)
dot.plots <- dot.plots & theme(axis.title = element_blank(), axis.text = element_text(size = 8)) 

## Figure S6
dev.new()
dot.plots + plot_layout(heights = unit(c(1.5), c("in")), widths = unit(c(1.4), c("in")))
...
vip + scatter1 + scatter2 + odds.scatter + DI.plot + DI.plot.GABA_1 + plot_layout(design = fig_layout, width = unit(c(20,15,15,20,15,20), "mm"), height = unit(c(30), "mm"))
...
# DI.plot <- DI.plot + plot_layout(width = unit(c(28), c("mm")), height = unit(c(40), c("mm")))
# DI.plot.GABA_1 <- DI.plot.GABA_1 + plot_layout(width = unit(c(34), c("mm")), height = unit(c(40), c("mm")))
# 
# 
# dev.new()
# odds.scatter + DI.plot + DI.plot.GABA_1
