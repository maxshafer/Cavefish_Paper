...
## This script identifies pairs of paralogs which are expressed in orthologous cell types, and computes the average 
## orthology confidence scores from ensembl for each subcluster for comparison between species specific genes and conserved genes
## Makes data for current Figure S5a and S5b
...
gene.lists.para[[2]] <- lapply(seq_along(clusters), function(x) {
  genes.1 <- setdiff(row.names(gene.lists.pos[[2]][[clusters[x]]]), row.names(gene.lists.pos[[1]][[clusters[x]]])) # species.1 specific genes
  genes.2 <- setdiff(row.names(gene.lists.pos[[3]][[clusters[x]]]), row.names(gene.lists.pos[[1]][[clusters[x]]])) # species.2 specific genes
...
gene.lists.para[[3]] <- lapply(seq_along(clusters), function(x) {
...
names(gene.lists.para[[2]]) <- clusters
names(gene.lists.para[[2]]) <- clusters
...
names(test) <- c("subcluster.conserved.combined", "subcluster.zebrafish.combined", "subcluster.astyanax.combined")
...
# Compute the mean of Orthology confidence score for paralogs expressed in zebrafish
para <- unlist(lapply(names(gene.lists.para$subcluster.zebrafish), function(x) {
  gocs <- mart.zeb[mart.zeb$Gene.name %in% row.names(gene.lists.para[["subcluster.zebrafish"]][[x]]),"Cave.fish.orthology.confidence..0.low..1.high."]
  mean(gocs[!(is.na(gocs))])
}))
markers.con <- unlist(lapply(names(gene.lists.pos$subcluster.conserved), function(x) {
  gocs <- mart.zeb[mart.zeb$Gene.name %in% row.names(gene.lists.pos[["subcluster.conserved"]][[x]]),"Cave.fish.orthology.confidence..0.low..1.high."]
  mean(gocs[!(is.na(gocs))])
}))
markers.zeb <- unlist(lapply(names(gene.lists.pos$subcluster.zebrafish), function(x) {
  gocs <- mart.zeb[mart.zeb$Gene.name %in% row.names(gene.lists.pos[["subcluster.zebrafish"]][[x]]),"Cave.fish.orthology.confidence..0.low..1.high."]
  mean(gocs[!(is.na(gocs))])
}))

df.conf <- data.frame(markers.con = markers.con, markers.zeb = markers.zeb, para = para)
row.names(df.conf) <- names(gene.lists.para$subcluster.conserved)
...
para <- unlist(lapply(names(gene.lists.para$subcluster.zebrafish), function(x) {
  gocs <- mart.zeb[mart.zeb$Gene.name %in% row.names(gene.lists.para[["subcluster.zebrafish"]][[x]]),"Cave.fish.Gene.order.conservation.score"]
  mean(gocs[!(is.na(gocs))])
}))
markers.con <- unlist(lapply(names(gene.lists.pos$subcluster.conserved), function(x) {
  gocs <- mart.zeb[mart.zeb$Gene.name %in% row.names(gene.lists.pos[["subcluster.conserved"]][[x]]),"Cave.fish.Gene.order.conservation.score"]
  mean(gocs[!(is.na(gocs))])
}))
markers.zeb <- unlist(lapply(names(gene.lists.pos$subcluster.zebrafish), function(x) {
  gocs <- mart.zeb[mart.zeb$Gene.name %in% row.names(gene.lists.pos[["subcluster.zebrafish"]][[x]]),"Cave.fish.Gene.order.conservation.score"]
  mean(gocs[!(is.na(gocs))])
}))

df.gene.order <- data.frame(markers.con = markers.con, markers.zeb = markers.zeb, para = para)
row.names(df.gene.order) <- names(gene.lists.para$subcluster.conserved)
...
names(orth_conf_metrics) <- c("Orthology confidence scores", "Gene order conservation scores")

saveRDS(orth_conf_metrics, file = paste("paralogs-orthology-conf-metrics_trinarized_a", a, "_b", b, "_f",f,".rds", sep = "")) 
