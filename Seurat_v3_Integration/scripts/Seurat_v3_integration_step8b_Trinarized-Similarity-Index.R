...
SI.sub <- data.frame(Subclustere = c(names(unlist(trin.SI.Subclusters))), values = c(unlist(trin.SI.Subclusters)))
...
calcCorrDriftIndex <- function(cell_type1 = cell_type1, cell_type2 = cell_type2, mart.1 = mart[[1]], mart.2 = mart[[2]]) {
  ...
  paralog.con <- union(mart.1$Zebrafish.paralogue.associated.gene.name[match(index, mart.1$Gene.name)], mart.2$Cave.fish.paralogue.associated.gene.name[match(index, mart.2$Gene.name)])
  paralog.2 <- union(mart.1$Zebrafish.paralogue.associated.gene.name[match(cell_type2, mart.1$Gene.name)], mart.2$Cave.fish.paralogue.associated.gene.name[match(cell_type2, mart.1$Gene.name)])
  paralog.1 <- union(mart.1$Zebrafish.paralogue.associated.gene.name[match(cell_type1, mart.1$Gene.name)], mart.2$Cave.fish.paralogue.associated.gene.name[match(cell_type1, mart.2$Gene.name)])
  
  paralog.1 <- paralog.1[paralog.1 %in% mart.2$Gene.name]
  paralog.2 <- paralog.2[paralog.2 %in% mart.1$Gene.name]
  
  a1 <- length(cell_type1[cell_type1 %in% union(paralog.con, paralog.2)]) # genes.1 that are paralogs of a conserved or species 2 gene (should be subtracted from genes.1 and added to con)
  a2 <- length(cell_type2[cell_type2 %in% union(paralog.con, paralog.1)])
  ...
  DI <- 1 - sqrt( abs( (1 - (length(index) / length(cell_type1))) * (1 - (length(index) / length(cell_type2))) ) )
  return(DI.corr)
}
...
# 
# DI.plots.035.hk <- DI.matrix + DI.matrix.scaled
# 
# 
