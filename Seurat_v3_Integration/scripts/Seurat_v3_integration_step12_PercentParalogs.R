...
calcParalog <- function(conserved = conserved, species.1 = species.1, species.2 = species.2, mart.1 = mart[[1]], mart.2 = mart[[2]], ngenes.1 = 32191, ngenes.2 = 25271, i = i) {
	...
	paralog.con <- union(mart.1$Zebrafish.paralogue.associated.gene.name[match(row.names(conserved[[i]]), mart.1$Gene.name)], mart.2$Mexican.tetra.paralogue.associated.gene.name[match(row.names(conserved[[i]]), mart.2$Gene.name)])
	paralog.1 <- union(mart.1$Zebrafish.paralogue.associated.gene.name[match(setdiff(row.names(species.1[[i]]), row.names(conserved[[i]])), mart.1$Gene.name)], mart.2$Mexican.tetra.paralogue.associated.gene.name[match(setdiff(row.names(species.1[[i]]), row.names(conserved[[i]])), mart.1$Gene.name)])
	paralog.2 <- union(mart.2$Zebrafish.paralogue.associated.gene.name[match(setdiff(row.names(species.2[[i]]), row.names(conserved[[i]])), mart.2$Gene.name)], mart.2$Mexican.tetra.paralogue.associated.gene.name[match(setdiff(row.names(species.2[[i]]), row.names(conserved[[i]])), mart.2$Gene.name)])
...
subtypes$Subtypes <- factor(subtypes$Subtypes, levels = c("Endothelial", "Erythrocytes", "Ciliated", "Ependymal", "Progenitors", "Oligodendrocyte_Precursor_Cells", "Oligodendrocytes", "GABA_0", "GABA_1", "GABA_2", "GABA_3", "GABA_4", "GABA_5", "Prdx1_Positive", "Glut_0", "Glut_1", "Glut_2", "Glut_3", "Glut_4", "Glut_5", "Glut_6", "Lymphatic", "Leucocytes", "Macrophages", "Microglia"))

subtypes.2 <- melt(subtypes[,13:18])

#### For subclusters!

paralog.numbers.sub <- lapply(seq_along(gene.lists.pos[[4]]), function(x) calcParalog(conserved = gene.lists.pos[[4]], species.1 = gene.lists.pos[[5]], species.2 = gene.lists.pos[[6]], i = x))

fisher.results.1 <- lapply(paralog.numbers.sub, function(x) fisher.test(matrix(data = x[1:4], ncol = 2, nrow = 2)))
fisher.results.1 <- do.call(rbind, fisher.results.1)
row.names(fisher.results.1) <- names(gene.lists.pos[[4]])
fisher.results.2 <- lapply(paralog.numbers.sub, function(x) fisher.test(matrix(data = x[5:8], ncol = 2, nrow = 2)))
fisher.results.2 <- do.call(rbind, fisher.results.2)
row.names(fisher.results.2) <- names(gene.lists.pos[[4]])

subclusters <- data.frame(do.call(rbind, paralog.numbers.sub))
row.names(subclusters) <- names(gene.lists.pos[[4]])
...
subclusters$Subtype <- factor(subclusters$Subtype, levels = c("Endothelial", "Erythrocytes", "Ciliated", "Ependymal", "Progenitors", "Oligodendrocyte_Precursor_Cells", "Oligodendrocytes", "GABA_0", "GABA_1", "GABA_2", "GABA_3", "GABA_4", "GABA_5", "Prdx1_Positive", "Glut_0", "Glut_1", "Glut_2", "Glut_3", "Glut_4", "Glut_5", "Glut_6", "Lymphatic", "Leucocytes", "Macrophages", "Microglia"))
