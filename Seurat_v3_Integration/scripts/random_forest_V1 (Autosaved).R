...
# Subset hypo.zeb so it's computational easier
hypo <- hypo.zeb
hypo.zeb <- SetAllIdent(hypo, id = "orig.ident")
hypo.zeb <- SubsetData(hypo.zeb, max.cells.per.ident = 1000)
...
# Have to remove some genes which are problematic (should do this up?)
genes.tfs <- genes.tfs[-grep("-", genes.tfs)]
genes.tfs <- genes.tfs[-grep(":", genes.tfs)]

# linear regression

lin.reg <- lm(log(sst1.1+1) ~ ., data = as.data.frame(as.matrix(t(hypo.train@data[c(genes.tfs, "sst1.1"),]))))
test.pred.lin <- exp(predict(lin.reg,as.data.frame(as.matrix(t(hypo.test@data)))))-1
hypo.test@data <- rbind(hypo.test@data, test.pred.lin)

# Decision tree (singular)
library(rpart)
library(rattle)

rt <- rpart(sst1.1 ~ ., data = as.data.frame(as.matrix(t(hypo.train@data[c(genes.tfs, "sst1.1"),]))))

test.pred.rtree <- predict(rt, as.data.frame(as.matrix(t(hypo.test@data))))
hypo.test@data <- rbind(hypo.test@data, test.pred.rtree)

# Random forest

rf <- randomForest(sst1.1 ~ ., data = as.data.frame(as.matrix(t(hypo.train@data[c(genes.tfs, "sst1.1"),]))), importance = T, ntree = 20)
imp <- as.data.frame(sort(randomForest::importance(rf)[,1],decreasing = TRUE),optional = T)

test.pred.forest <- predict(rf, as.data.frame(as.matrix(t(hypo.test@data))))
hypo.test@data <- rbind(hypo.test@data, test.pred.forest)

# Plot the original value plus predicated value for 3 models, and the genes which are most predictive
dev.new()
DotPlot(object = hypo.test, genes.plot = c(row.names(imp)[1:20], "sst1.1", "test.pred.lin", "test.pred.rtree", "test.pred.forest"), group.by = "SubclusterType", plot.legend = TRUE, x.lab.rot = TRUE) + theme(legend.position = "right")
