...
corr <- data.frame(corr.surface, corr.cave)
corr$Subclusters <- row.names(corr)
corr$Subtypes <- hypo.integrated@meta.data$integrated_Subtype[match(corr$Subclusters, hypo.integrated@meta.data$integrated_SubclusterType)]
corr$diff <- ifelse(abs(corr$corr.surface - corr$corr.cave) > 0.6, "yes", "no")
corr$diff2 <- corr$corr.surface - corr$corr.cave

corr$Subtypes <- factor(corr$Subtypes, levels = c("Endothelial", "Erythrocytes", "Ciliated", "Ependymal", "Progenitors", "Oligodendrocyte_Precursor_Cells", "Oligodendrocytes", "GABA_0", "GABA_1", "GABA_2", "GABA_3", "GABA_4", "GABA_5", "Prdx1_Positive", "Glut_0", "Glut_1", "Glut_2", "Glut_3", "Glut_4", "Glut_5", "Glut_6", "Lymphatic", "Leucocytes", "Macrophages", "Microglia"))

ggplot(corr, aes(x = corr.surface, y = corr.cave, color = Subtypes)) + geom_point() + xlab("D. rerio vs A. mexicanus - Surface") + ylab("D. rerio vs A. mexicanus - Cave") + theme(axis.text = element_text(size = 8), text = element_text(size = 10)) + geom_text_repel(data = test[test$diff == "yes",], aes(label = Subclusters), size = 2) + guides(color = F)


ggplot(corr, aes(x = Subtypes, y = diff2, color = Subtypes)) + geom_jitter() + guides(color = F) + theme(axis.title = element_text(size = 10), axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1), axis.text = element_text(size = 8)) + xlab("") + ylab("Similary D. rerio \n vs A. mexicanus Surface") + ylim(-1,1) + geom_hline(yintercept = 0, linetype = "dashed", color = "black") + geom_hline(yintercept = 0.5, linetype = "dashed", color = "red") + geom_hline(yintercept = -0.5, linetype = "dashed", color = "red") + coord_flip()
